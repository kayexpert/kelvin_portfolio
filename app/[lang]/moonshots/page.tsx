...
const get_moonshots = async () => {
  const res = await fetch(`${process.env.SERVER_URL}/api/moonshots`)
  return await res.json() || []
}

export async function generateMetadata({ params }: any): Promise<Metadata> {
  const t = await getTranslations('moonshots_page')
  return {
  title: `${t('meta_title')}`,
  ...
  openGraph: {
    ...
    title: `${t('meta_title')}`,
    description: t('site_description'),
    url: `${process.env.SERVER_URL}/moonshots`,
    images: ['favicon.png']
  },
  twitter: {
...
export default async function Moonshots ({  }: any) {
  const moonshots = await get_moonshots()
  const t = await getTranslations('moonshots_page')
  ...
  return (
    <>
      <Flex  mb={'5'} gap={'5'} align={'center'}>
        <Heading style={{ viewTransitionName: `page_title`, whiteSpace: 'nowrap' }}>{t('moonshots_title')}</Heading>
        <Separator size={'4'}  />
      </Flex >
      <Grid columns={{ initial: '1', sm: '2' }} gap={'5'}>
        {moonshots?.map((val: any, ind: any) => (
          <Card size={'3'} key={ind} >
            <Flex direction={'column'} gap={'2'} >
              <Flex flexShrink={'0'} gap={'5'} align={'center'}>
                <Flex direction={'column'} >
                  <Text size={'4'} style={{ viewTransitionName: `moonshot_${val.id}_title` }}>{proj_t(`${val.id}.title`)}</Text>
                  <Text size={'2'} style={{ viewTransitionName: `moonshot_${val.id}_company` }}>{proj_t(`${val.id}.company`)}</Text>
                </Flex >
              </Flex>
              <Flex flexGrow={'1'} direction={'column'} gap={'5'} align={'start'}>
                <Text size={'1'} style={{ viewTransitionName: `moonshot_${val.id}_scope` }}>{proj_t(`${val.id}.scope`)}</Text>
                <Flex gap={'2'}>
                  <Link href={`/moonshots/${val.id}`}><Button size={'1'} highContrast>{t('read_more')}</Button></Link>
                  {val.link && 
        ...
        {/* <Box   p={'5'} minHeight={'200px'}>
          <Flex direction={'column'} gap={'2'} >
            ...
            <Flex flexGrow={'1'} direction={'column'} gap={'5'} align={'start'}>
              ...
              <Flex >
                <Button as={NextLink} href={'/moonshot/1234'} size={'1'}>Read More ...</Button>
